...
def generate_140(ProjectID, formtype, other_objects):
    ...
    if result:
        if True:
            ...
            # with open(os.path.join(target_dir,'Form I-140.pdf'), 'wb') as pdf:
            ...
            data_dict2 = [
                ...
                },
                {
                    ...
                    "FieldValue": result.l_middlename if result.l_middlename is not None else iff(iff(results2.AttorneyAgentMiddleInitial))
                },
                {
                    "FieldName": "1lawfirm.O.Addr123",
                    "FieldCalcOverride": True,
                ...
                },
                {
                    ...
                    "FieldValue": ('Form I-140 - '+str(result.b_lastname if result.b_lastname is not None else results2.BeneficiaryLastName)+', '+str(result.b_firstname if result.b_firstname is not None else results2.BeneficiaryFirstName)+' '+str(result.b_middlename if result.b_middlename is not None else iff(results2.BeneficiaryMiddleName))).strip()
                },
                {
                    "FieldName": "1ben.LName",
            ...
            pdf_base64_g28 = api_response['ResultData']['PDF']
            # print()
            # with open(os.path.join(target_dir,'Form G-28_for_I-140.pdf'), 'wb') as pdf:
            #     pdf.write(base64.b64decode(pdf_base64))
    ...
    # shutil.move(op.join(source_dir, file_name), op.join(processed_dir, file_name))
    pdf_merge_sp(data_dict, formtype, result, results2,most_recent_entry_rec,most_recent_notice,parent_dir,iff,change_date,qpdf_merger,get_files,download_files,upload_to_sharepoint, pdf_base64, pdf_base64_g28)



def pdf_merge_sp(data_dict, formtype, result, results2,most_recent_entry_rec,most_recent_notice,parent_dir,iff,change_date,qpdf_merger,get_files,download_files,upload_to_sharepoint,pdf_base64, pdf_base64_g28):
    ...
    with open(os.path.join(target_dir,'Form G-28_for_I-140.pdf'), 'wb+') as pdf:
        pdf.write(base64.b64decode(pdf_base64_g28))
    ...
    if result.ben_Salutation:
        ben_salutation = str(result.ben_Salutation).replace('\n','')
    else:
    ...
    rep_dic = {
        ...
        "beneficiaryfullnamehere": f'{result.b_firstname} {iff(result.b_middlename)}, {result.b_lastname}',
        ...
        "88888": ben_salutation,
        "77777": f'{result.l_firstname} {iff(result.l_middlename)}, {result.l_lastname}',
        ...
        'clientfullnamehere': f'{result.b_firstname} {iff(result.b_middlename)}, {result.b_lastname}',
        ...
        'clientprojectprimatyhere': f'{result.l_firstname} {iff(result.l_middlename)}, {result.l_lastname}',
        '33333': f'{result.l_firstname} {iff(result.l_middlename)}, {result.l_lastname}',
        ...
        'projectprimaryattorneyhere': f'{result.l_firstname} {iff(result.l_middlename)}, {result.l_lastname}',
    ...
    # 66666 - projectsponsortitlehere
    # 77777 - primaryprojectattorneyhere
    ...
    if op.exists(op.join(support_doc_dir,'I-140 Cover Letter.docx')):

        cover_doc = docx.Document(op.join(support_doc_dir,'I-140 Cover Letter.docx'))
    ...
    if op.exists(op.join(support_doc_dir,'I-140 - Employer Support Letter.docx')):
        support_doc = docx.Document(op.join(support_doc_dir,'I-140 - Employer Support Letter.docx'))
    ...
    if op.exists(op.join(support_doc_dir,"I-140 - Initial QC Doc.docx")):
        
        initial_qc_doc = docx.Document(op.join(support_doc_dir,"I-140 - Initial QC Doc.docx"))
    ...
    pdfs.append(op.join(support_doc_dir, 'I-140 Supporting Docs Final Packet.pdf'))
    ...
    os.remove(op.join(target_dir, 'Form G-28_for_I-140.pdf'))
    os.remove(op.join(target_dir, 'Form I-140.pdf'))
