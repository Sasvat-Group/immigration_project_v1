...
def generate_765(CaseID, formtype, other_objects):
    ...
    if result:
        ...
        ren_ext = ['renewal', 'ren.', 'ren', 'initial', 'init.',
                   'init', 'extension', 'ext', 'extn.', 'extn']
        ...
        if result.PTag1_Val and result.PTag3_Val:
            ...
            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "aos advance parole"):
                form_765_code = "c 0 9"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "asylum ead"):
                form_765_code = "c 8"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "b-1 ead (domestic servant of niv)"):
                form_765_code = "c 17 i"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "b-1 ead (domestic servant of usc)"):
                form_765_code = "c 17 ii"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "b-1 ead (emp of foreign airline)"):
                form_765_code = "c 17 iii"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "compelling circumstances"):
                form_765_code = "c 3 6"
            ...
            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "f-1 opt - not stem eligible"):
                form_765_code = "N/A"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "f-1 stem ead"):
                form_765_code = "c 3 C"

            elif (result.PTag1_Val.lower().strip() in ren_ext) and (result.PTag3_Val.lower().strip() == "f-1 student - not work authorized"):
                form_765_code = "N/A"
        ...
        data_dict = [
            ...
            {
                "FieldName": "USIMM82541.txt3.76",
                "FieldValue": result.b_USCISNumber if result.b_USCISNumber else ''
            },
            {
            ...
            },
            {
                ...
                "FieldValue": most_recent_entry_rec.EntryCity + ', '+most_recent_entry_rec.EntryState if (most_recent_entry_rec.EntryCity and most_recent_entry_rec.EntryState) else ''
            },
            {
...

            {
                ...
                "FieldValue": AptSteFlr(current_res_address.AddressUnitType) if current_res_address.AddressUnitType and not mail_addr_vs_phys_addr else ''
            },
            {
            ...
            },
            {
                ...
                "FieldValue": clean_num(result.SSNNumber) if result.SSNNumber else 'N/A'
            },
            {
                "FieldName": "1ben.I94.ArrivalDate",
                "FieldValue": (most_recent_entry_rec.EntryDate).strftime('%m/%d/%Y') if most_recent_entry_rec.EntryDate is not None else ''
            },
            {
            ...
            },
            {
                ...
                "FieldValue": 'Yes' if ((str(most_recent_entry_rec.I94StatusType.lower() in ('j1', 'j2'))) or (result.HasPreviouslyHeldJVisaStatus == 1)) else 'No'
            },
            ...
            if ben_high_edu:
                data_dict.extend([
                    ...
                    },
                    {
                        ...
                        "FieldValue": result.p_EmployerEVerifyName if result.p_EmployerEVerifyName else iff(ben_cur_emp.EmployerEVerifyName) if ben_cur_emp else ''
                    },
                    {
        ...
        if form_765_code == 'c 8':
            ...
            data_dict.extend([
...

                {
                    ...
                    "FieldValue": change_date(result.AsylumReportedDateToDHS) if result.AsylumReportedDateToDHS else 'No'
                }
            ])

            if most_recent_entry_rec.IsInspectedAdmitted == False:
                data_dict.append({
        ...
        data_dict2 = [
            ...
            },
            {
                ...
                "FieldValue": ('Form I-765 - '+str(result.b_lastname if result.b_lastname is not None else results2.BeneficiaryLastName)+', '+str(result.b_firstname if result.b_firstname is not None else results2.BeneficiaryFirstName)+' '+str(result.b_middlename if result.b_middlename is not None else iff(results2.BeneficiaryMiddleName))).strip()
            },
            {
                "FieldName": "1ben.LName",
...
def pdf_merge_sp(data_dict, formtype, result, results2,most_recent_entry_rec,most_recent_notice,parent_dir,iff,change_date,qpdf_merger,get_files,download_files,upload_to_sharepoint):
    ...
    pdfs.append(op.join(support_doc_dir, 'I-765 Supporting Docs Final Packet.pdf'))
    ...
    os.remove(op.join(target_dir, 'Form G-28_for_I-765.pdf'))
    os.remove(op.join(target_dir, 'Form I-765.pdf'))
