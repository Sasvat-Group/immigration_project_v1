    ...
    if result:
        if True:
            ...
            dependents = cursor.execute(
                '''select *,DATEDIFF(YY,BirthDate,GETDATE()) as Age from [dbo].[Beneficiary] where PrimaryBeneficiaryID = '{}' and not (LOWER(BirthCountry) Like '%america%' or LOWER(BirthCountry) Like '%u.s.a%' or LOWER(BirthCountry) Like '%usa%' ) and not (LOWER(CitizenshipCountry) Like '%america%' or LOWER(CitizenshipCountry) Like '%u.s.a%' or LOWER(CitizenshipCountry) Like '%usa%' ) and ( (lower(RelationType) in('child','son','daughter','biological son','biological daughter','biological child') and ((DATEDIFF(YY,BirthDate,GETDATE()))<21))) '''.format(
            ...
            data_dict = [
                ...
                },
                {
                    ...
                    "FieldValue": iff(
                        result.LCAWorkLocation1_City) + '   -   See attached LCA for additional work locations' if result.LCAWorkLocation1_Address1 else ''
                },
                {
            ...
            if project_cat == 'h1-b regular cap':
                ...
                value = 2
            elif word.__contains__('h-1b') and re.search(
                    "change of employer|Extension of Status|Amendment|change of status", word, re.IGNORECASE):
                value = 4
                ...
                if project_cat in (
                        'h1-b change of employer', 'h1-b ext', 'h1-b amendment only', 'h1-b amendment with ext'):
                    data_dict.append({
