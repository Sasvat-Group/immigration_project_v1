        ...
        if True:
            ...
            data_dict = [
                ...
                },
                {
                    ...
                    "FieldValue": change_date(result.IntendedDepartureDate)
                },
                {
                    "FieldName": "USIMM82537.txt1.56",
                    "FieldValue": str(
                        result.trip_length) if result.IntendedReturnDate and result.IntendedDepartureDate else ''
                },
                {
                ...
                {  # p3 4a
                    ...
                    "FieldValue": 'Yes' if (
                                result.RefugeeTravelDocumentIssueDate or result.ReEntryPermitIssueDate) else 'No'
                },
                {
                    "FieldName": "USIMM82537.txt1.82",
                    "FieldValue": change_date(
                        result.RefugeeTravelDocumentIssueDate) if result.RefugeeTravelDocumentIssueDate else change_date(
                        result.ReEntryPermitIssueDate)
                },
                {
                    "FieldName": "USIMM82537.txt1.86",
                    "FieldValue": 'See Attached.' if (
                                result.RefugeeTravelDocumentIssueDate or result.ReEntryPermitIssueDate) else ''
                },
...

                {
                    ...
                    "FieldValue": iff(result.PurposeOfTravel)
                },

                {
                    "FieldName": "User.D937.CountriesIntendToVisit.multiline.1",
                    "FieldValue": ', '.join(result.VisitingCountries.split(';')) if result.VisitingCountries else ''
                },
                {
            ...
            if result.PTag3_Val:
                ...
                if re.search("reentry|re-entry", PTag3_Val):
                    ...
                    value = 4
                elif (re.search("advance parole", PTag3_Val)) and not (
                US_keyword.fullmatch(current_res_address.Country)):
                    value = 5
                data_dict.append(
                    {
