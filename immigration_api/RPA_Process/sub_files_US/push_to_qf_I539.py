...
def generate_539(CaseID, formtype, other_objects):
    ...
    if result:
        if True:
            ...
            data_dict = [
                ...
                },
                {
                    ...
                    "FieldValue": AptSteFlr(current_res_address.AddressUnitType) if AptSteFlr(
                        current_res_address.AddressUnitType) else ''
                },
                {
...

                {
                    ...
                    "FieldValue": current_res_address.Country if str(
                        current_res_address.Country).lower().strip() not in ('usa', 'u.s.a', 'america', 'united states of america') else last_perm_address_abroad.Country
                },
                {
                ...
                },
                {
                    ...
                    "FieldValue": perm_foreign_address.StateProvince if str(
                        mailing_address.Country).lower().strip() in ('usa', 'u.s.a', 'america',
                                                                     'united states of america') else ''
                },
                {
                ...
                },
                {
                    ...
                    "FieldValue": (
                            str(result.FirstName if result.FirstName is not None else results2.PetitionerContactFirstName) + ' ' + str(
                ...
                {
                    "FieldName": "1ben.DOB",
                    "FieldValue": result.BirthDate.strftime(
                        '%m/%d/%Y') if result.BirthDate is not None else results2.BeneficiaryDateofBirth.strftime(
                        '%m/%d/%Y')
                },
                {
                ...
                },
                {
                    ...
                    "FieldValue": 'Yes' if result.AosProjectFiledDate or (result.ServiceType.lower().strip() in (
                        'aos application', 'adjustment of status', '485')) else 'No'
                },
                {
            ...
            response = requests.post(
                'https://websvcs.quikforms.com/rest/quikformsengine/qfe/execute/pdf', headers=headersAPI,
                data=data_json)
            ...
            if len(dependents) > 0:
                ...
                for dep in dependents:
                    data_539A = [
                        ...
                        {
                            "FieldName": "1ben.NonImmigExpiry",
                            "FieldValue": dep.I94ExpirationDate.strftime('%m/%d/%Y') if str(
                                dep.I94StatusType).lower() not in ('j1', 'j2', 'f1', 'f2') else 'N/A'
                        },
                        {
        ...
        pdf_merge_sequence(data_dict, dependents, formtype, result, results2, most_recent_entry_rec, most_recent_notice,
                           parent_dir, iff, change_date, qpdf_merger)
    else:
        print('No Matching record in db to Process..')


def pdf_merge_sequence(data_dict, dependents, formtype, result, results2, most_recent_entry_rec, most_recent_notice,
                       parent_dir, iff, change_date, qpdf_merger):
    os.chdir(parent_dir)
    ...
    if op.exists(support_doc_dir):
        ...
        for dep in dependents:
            shutil.move(os.path.join(parent_dir, f'Form I-539A_{dep.FirstName}.pdf'),
                        os.path.join(target_dir, f'Form I-539A_{dep.FirstName}.pdf'))
